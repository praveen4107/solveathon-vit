...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  useEffect(() => {
    ...
    if (bloomRef.current) {
      ...
      const bloomInternal = bloomRef.current as unknown as {
        luminanceMaterial?: { threshold: number; smoothing: number };
      };
      if (bloomInternal.luminanceMaterial) {
        bloomInternal.luminanceMaterial.threshold = bloomThreshold;
        bloomInternal.luminanceMaterial.smoothing = bloomSmoothing;
      }
    }
  ...
  // Default to `relative` positioning only if the caller didn't provide a position utility.
  // This prevents Tailwind's position utility ordering from accidentally overriding `fixed`/`absolute`.
  const hasPositionClass =
    typeof className === 'string' && /\b(static|fixed|absolute|relative|sticky)\b/.test(className);
  const rootClassName = `${hasPositionClass ? '' : 'relative '}w-full h-full overflow-hidden ${className ?? ''}`.trim();
